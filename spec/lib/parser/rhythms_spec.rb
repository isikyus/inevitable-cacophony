...
RSpec.describe InevitableCacophony::Parser::Rhythms do
	...
	describe 'parsing rhythms with non-ASCII names' do
		let(:rhythm_text) do
			...
			# Musical form generated by Dwarf Fortress, copyright Bay 12 Games
			<<-RHYTHM
			The itho rhythm is made from three patterns: the èle (considered the primary), the aríle, and the amama. The patterns are to be played over the same period of time, concluding together regardless of beat number.

			The èle rhythm is a single line with three beats. The beats are named timafi (spoken ti), emu (wm), and úpe (úp). The beat is stressed as follows:

			| - - x |

			The aríle rhythm is a single line with twenty-four beats divided into eight bars in a 3-3-3-3-3-3-3-3 pattern. The beat is stressed as folows:

			| - - x | x X x | - x - | x x'! | x X x | x - - | x - x | X x`x |

			The amama rhythm is a single line with sixteen beats divided into two bars in a 8-8 pattern. The beat is stressed as follows:
